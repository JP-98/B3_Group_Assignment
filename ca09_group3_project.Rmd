...
msummary(model2)
```
Adjusted R squared is 0.2817, which is a marginal improvement on the previous model.
...
car::vif(model3)
```
Adjusted R-squared is 0.392, greatly improved by adding `bathrooms`, `bedrooms`, and size of the house (`accomodates`) 
...
msummary(model3)
```
By deleting bathrooms and beds, the Adjusted R-squared improved by a very small margin.
...
msummary(model4)
```
Adjusted R-squared is 0.4019, a little improved by adding `(host_is_superhost`) 
...
msummary(model5)
```
The adjusted R squared is unchanged from the modified version of Model 4, but we have an insignificant predictor in `instant_bookable` so we are reluctant to use this model going forward.
