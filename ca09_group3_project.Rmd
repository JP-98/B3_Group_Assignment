---
title: "Final Group Project: AirBnB analytics - Raw Code"
date: "18 Oct 2021"
...
We first load the data and parse the price into an integer:
...
The following code, having downloaded a dataframe `listings` with all AirbnB listings in Vienna, will plot on the map all AirBnBs where `minimum_nights` is less than equal to four (4). 
...
We then conduct a thorough EDA via three methods:
...
According to our hypothesis, the following variables will potentially affect the price. 
...
Then, we plot the distribution of pirce. As the distribution is significantly right skewed, we use the log function to aptly transform the price data.
...
```
As the log price is still a bit right skewed, we then plot the 5% and 95% percentile of the price. This gives us approximate price upper and lower bounds of €20 and €169 respectively. After we filter the date outside of this bondary, we then plot the lnprice as below.
...
```
Now the logged price seems normally distributed, and we use lnprice as our dependent variable to construct our model.


### Non-numerical to numerical: Bathrooms, host_response_rate, host_acceptance_rate

We then change the non-numerical variables such as bathrooms, host_response_rate, and host_acceptance_rate to numericals.
...
Next, we look at the variable `property_type`. We use the `count` function to determine how many categories there are their frequency. The top 4 most common property types are "Entire rental unit", "Private rom in rental unit", " Entire condominium (condo)", and "Entire serviced apartment". The four types account for 91.19% proportion of the total listings.
...
Its residuals all randomly scatter around its mean of 0, and are approximately normal as they appear to simulate a white noise distribution with no clear pattern as the target variable varies. The QQ plot is approximately along the 45 degree line also, with most residual points well fitted. It does not appear that we require heteroskedasticity robust standard errors.
