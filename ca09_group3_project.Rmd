...
listings <- vroom("http://data.insideairbnb.com/austria/vienna/vienna/2021-09-12/data/listings.csv.gz") %>% 
       clean_names()
```
```{r}
...
```{r}
# 1. Use Glimpse from the Dplyr package to get an initial look at the raw data
dplyr::glimpse(listings)
```

```{r}
#2. Generate summary statistics with mosaic and skimr packages
skimr::skim(listings)
```
...
```{r}
# Visualise the Data
listings %>% ggplot(aes(x=lnprice)) + 
  geom_density()
```
```{r}
confint(listings$price, mean, level = 0.9)

```
This gives us approximate price upper and lower bounds of €20 and €170 respectively.

```{r}
# Remove positive outliers and non-sensical/irrelevant values left of the mean to build a more meaningful analsysis
listings <- listings %>% 
  filter(between(lnprice, 3, 5.13), na.RM=TRUE)
...
In the original raw data, there are:
 - 74 Variables
 - 11,583 Observations

After filtering the data based on log price (to remove NA price values and outliers), we have:
 - 10,418 observations
...
- Which are categorical or *factor* variables (numeric or character variables with variables that have a fixed and known set of possible values?
 - Logical/Boolean Variables are categorical (True/False)
 - Character variables are also categorical
...
SELECT VARIABLES BASED ON LOGIC
